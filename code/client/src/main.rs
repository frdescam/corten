...
fn main() {
    ...
    request_airdrop(&rpc_client, &sender_keypair.pubkey(), 1.0).unwrap();

    // Requesting airdrop for the receiver
    request_airdrop(&rpc_client, &receiver_pubkey, 1.0).unwrap();

    // Printing the balances
    let sender_balance = check_balance(&rpc_client, &sender_keypair.pubkey()).unwrap();
    let receiver_balance = check_balance(&rpc_client, &receiver_pubkey).unwrap();
